...
const Demo = () => {
  ...
  const  handleSubmit = async (e)=>{
      ...
      articleSummary(article).then((data)=>{
        // console.log(data);
        setSummary(data)
      }).then(()=>{
    ...
    <section className='mt-16 w-full max-w-xl' >
      ...
      <div className='flex flex-col w-full gap-2' >
        <form className='relative flex justify-center items-center'  >
          ...
          <input type='text' placeholder='Enter your article' required className='url_input peer' value={article} onChange={(e) => addData(e)}/>
          <button type='Submit' className='submit_btn peer-focus:border-gray-700 peer-focus:text-gray-700' onClick={(e)=>handleSubmit(e)} > 🔍 </button>
        </form>
        {/* Browser articleHistory */}
       {articleData.length==0?( <div className='flex flex-col gap-1 max-h-60 overflow-y-auto' >
